...
router.post('/', async (req, res) => {
  ...
  const id = uuidv4();
  await query(
    `INSERT INTO assets (id, user_id, type, name, data, value, currency, source)
...
router.put('/:id', async (req, res) => {
  const { type, name, data, value, currency, source } = req.body;
  const existing = await query('SELECT * FROM assets WHERE id = $1 AND user_id = $2', [req.params.id, req.userId]);
  if (existing.rows.length === 0) return res.status(404).json({ error: 'Asset not found' });

  await query(
    `UPDATE assets SET type = $1, name = $2, data = $3, value = $4, currency = $5, source = $6, updated_at = CURRENT_TIMESTAMP
...
router.delete('/:id', async (req, res) => {
  const result = await query('DELETE FROM assets WHERE id = $1 AND user_id = $2 RETURNING id', [req.params.id, req.userId]);
